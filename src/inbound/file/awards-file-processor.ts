...
export class AwardsFileProcessor implements OnModuleInit {
    private lines: { year: number, title: string, studios: string, producers: string, winner: string }[] = [];
    ...
    private getAwardsFileReadableStream(): internal.Transform {
        ...
        const readableStream = createReadStream(movieFilePath, { encoding: 'utf-8' })
            .pipe(csvParser({ headers: false, separator: ';', raw: false, skipLines: 1 }));
    ...
    private registerEvents(awardsReadableStream: internal.Transform) {
        awardsReadableStream.on('data', async (row) => {
            ...
            this.lines.push({ year: parseInt(year), title, studios: studiosNames, producers: producersNames, winner });
        });
        ...
        awardsReadableStream.on('end', async () => {
            ...
            for (const line of this.lines) {
                ...
                const movie = await this.createMovie.execute(awardEdition.id, line.title);
                await this.createStudios.execute(movie.id, line.studios);
                const producers = await this.createProducers.execute(line.producers);
                if (line.winner === 'yes') {
                    await this.createWinners.execute({ awardEditionId: awardEdition.id, producerIds: producers.map(p => p.id) });
                }
            }
        });
