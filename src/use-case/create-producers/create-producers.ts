...
export class PerformCreateProducers implements CreateProducers {
    constructor(
        @Inject(TypeORMProducersRepository) private producerRepository: ProducersRepository
    ) { }
    ...
    async execute(producers: string): Promise<CreateProducers.Response[]> {
        ...
        for (const producer of producersNamesList) {
          const producerFound = await this.producerRepository.findProducersByName(producer.trim());
          if (producerFound) {
            producersData.push(producerFound);
            continue;
          }
          const result = await this.producerRepository.saveProducers({ name: producer.trim() });
