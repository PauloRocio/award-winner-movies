...
export class PerformCreateStudios implements CreateStudios {
    constructor(
        @Inject(TypeORMStudiosRepository) private studioRepository: StudiosRepository
    ) { }

    async execute(movieId: number, studios: string): Promise<CreateStudios.Response[]> {
        const studiosData: CreateStudios.Response[] = [];
        const studiosNamesList = studios.split(',');
        for (const studio of studiosNamesList) {
          const studioFound = await this.studioRepository.findStudiosByName(studio.trim());
          ...
          const result = await this.studioRepository.saveStudios({ name: studio.trim(), movieId });
