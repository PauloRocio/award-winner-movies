...
export class PerformCreateAwardEdition implements CreateAwardEdition {
  constructor(
    @Inject(TypeORMAwardEditionsRepository)
    private awardEditionsRepository: AwardEditionsRepository,
  ) {}

  async execute(year: number) {
    const awardEdition =
      await this.awardEditionsRepository.findAwardEditionByYear(year);

    if (awardEdition) return awardEdition;

    return this.awardEditionsRepository.saveAwardEdition({
      year: year,
      title: `${year} Award Edition`,
    });
  }
}
