...
export class PerformCreateWinners implements CreateWinners {
    ...
    async execute(params: CreateWinners.Params): Promise<CreateWinners.Response[]> {
        const winnersData: CreateWinners.Response[] = [];
        
        for (const producerId of params.producerIds) {
            const winnerFound = await this.winnersRepository.findWinnersByProducerIdAndAwardEditionId({ producerId, awardEditionId: params.awardEditionId });
          const result = await this.winnersRepository.saveWinners({ producerId, awardEditionId: params.awardEditionId });
