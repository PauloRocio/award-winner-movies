...
type WinnersGrouped = { producerId: number; interval: number, previous: number, next: number };

type ProducerWins = { producer: string; interval: number, previousWin: number, followingWin: number };

type Response = { min: ProducerWins[], max: ProducerWins[] };
@Injectable()
export class PerformGetWinners implements GetWinners {
    constructor(
        @Inject(TypeORMWinnersRepository) private readonly winnersRepository: WinnersRepository,
        @Inject(TypeORMProducersRepository) private readonly producersRepository: ProducersRepository,
    ) { }

    async execute(): Promise<Response> {
        try {
            const awardEditionsAndWinners = await this.winnersRepository.findWinners();
            let result = this.buildWinners(awardEditionsAndWinners);
    ...
    private buildWinners(awardEditionsAndWinners: WinnersRepository.FindWinners.Result): { producerId: number, years: number[] }[] {
        let result: { producerId: number, years: number[] }[] = [];
        awardEditionsAndWinners.forEach((awardEditionAndWinner) => {
            ...
            const producer = result.find((producer) => producer.producerId === producerId);
    ...
    private groupWinners(winners: { producerId: number, years: number[] }[]): WinnersGrouped[] {
    ...
    private async buildProducerAndAwards(win: WinnersGrouped): Promise<ProducerWins> {
        const producer = await this.producersRepository.findById({ id: win.producerId });
        return {
